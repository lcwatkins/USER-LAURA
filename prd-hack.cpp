...
void PRD::command(int narg, char **arg)
{
  ...
  if (me_universe == 0 && universe->uscreen)
    fprintf(universe->uscreen,"!!! FIX\n");
  fix_event->store_state_quench();
  if (me_universe == 0 && universe->uscreen)
    fprintf(universe->uscreen,"!!! QUENCH\n");
  quench();
  ncoincident = 0;
  if (me_universe == 0 && universe->ulogfile)
    fprintf(universe->ulogfile,"BEFORE SHARE\n");
  share_event(0,0,0);
  if (me_universe == 0 && universe->ulogfile)
    fprintf(universe->ulogfile,"AFTER SHARE\n");
  ...
  if (me_universe == 0 && universe->uscreen)
    fprintf(universe->uscreen,"START\n\n");
  ...
  while (istep < nsteps) {
    ...
    fprintf(universe->uscreen,"CHECK EVENT \n\n");
    ireplica = check_event();

    // hopefully write 
//    share_event(ireplica,0,0);
