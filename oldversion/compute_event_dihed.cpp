...
//  double **x = atom->x;
  ...
  for (atom2 = 0; atom2 < nlocal; atom2++) {
    ...
    for (i = 0; i < nd; i++) {
      ...
      // compute dihedrals for current and saved state
/*
      dih = compute_dihedral(atom1, atom2, atom3, atom4, 0);
      dihev = compute_dihedral(atom1, atom2, atom3, atom4, 1);

      fprintf(universe->uscreen,"%i, %i, %i, %i\n", atom1, atom2, atom3, atom4);
      fprintf(universe->uscreen,"Now: %f, prev: %f \n", dih, dihev);

      //add region stuff??
      // need to set a difference squared param
      ddih = dih - dihev;
      ddihsq = ddih*ddih;
      if (ddihsq >= 15*15) {
        event = 1.0;
...
//        fprintf(universe->uscreen,"event coords");
//      } else {
//        x = atom->x ;
//        fprintf(universe->uscreen,"current coords");
//      }
      ...
      fprintf(universe->uscreen,"dih: %f",dih);
